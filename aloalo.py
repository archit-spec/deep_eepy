...
# Create tensors in float32 first, then convert to float8_e4m3fn
lhs = (
    torch.randn((m, k), dtype=torch.float32, device="cuda").to(torch.float8_e4m3fn), 
    torch.randn((m, (k + 127) // 128), dtype=torch.float32, device="cuda")
)
rhs = (
    torch.randn((n, k), dtype=torch.float32, device="cuda").to(torch.float8_e4m3fn), 
